...
void FloodFilledImage::addFloodFill(ImageTraversal & traversal, ColorPicker & colorPicker) {
  /** @todo [Part 2] */
  //Implement a vector or array to store traversal and colorpicker
...
Animation FloodFilledImage::animate(unsigned frameInterval) const {
  ...
  for(unsigned long i =0; i < tv.size(); i++){
    ...
    for (ImageTraversal::Iterator it = traversal->begin(); it != traversal->end(); ++it){
      HSLAPixel & change = floodpng.getPixel(it.current.x, it.current.y);
      change = colorPicker->getColor(it.current.x, it.current.y);
      count++;
